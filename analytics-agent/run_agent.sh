...
if test -d shutter_analytics_agent; then
  echo "Removing previous agent build"
  rm -r shutter_analytics_agent
fi
...
# Fetch the agent
autonomy fetch --local --agent isotrop/shutter_analytics_agent
